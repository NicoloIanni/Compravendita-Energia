...
async function up(queryInterface) {
    ...
    if (slots.producerId && !slots.producerProfileId) {
        ...
        await queryInterface.removeConstraint("ProducerSlots", "fk_producerslots_producerId_profiles_id");
        // rinomina colonna
        await queryInterface.renameColumn("ProducerSlots", "producerId", "producerProfileId");
    ...
    if (reservations.producerId && !reservations.producerProfileId) {
        ...
        try {
            await queryInterface.removeConstraint("Reservations", "fk_reservations_producerId_profiles_id");
        }
        catch { }
        await queryInterface.renameColumn("Reservations", "producerId", "producerProfileId");
...
async function down(queryInterface) {
    ...
    if (slots.producerProfileId && !slots.producerId) {
        await queryInterface.removeConstraint("ProducerSlots", "fk_producerslots_producerProfileId_profiles_id");
        await queryInterface.renameColumn("ProducerSlots", "producerProfileId", "producerId");
        await queryInterface.addConstraint("ProducerSlots", {
            fields: ["producerId"],
            type: "foreign key",
    ...
    if (reservations.producerProfileId && !reservations.producerId) {
        try {
            await queryInterface.removeConstraint("Reservations", "fk_reservations_producerProfileId_profiles_id");
        }
        catch { }
        await queryInterface.renameColumn("Reservations", "producerProfileId", "producerId");
        try {
            await queryInterface.addConstraint("Reservations", {
                fields: ["producerId"],
                type: "foreign key",
